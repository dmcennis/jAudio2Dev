...
public class AudioSamples implements AudioInput {
    ...
    public AudioSamples prototype(String id, double[][] channels, float samplingRate, Properties props) {
        try{
            AudioSamples ret = new AudioSamples(channels,samplingRate,id,false);
            return ret;
        }catch(Exception e){
            Logger.getLogger(AudioSamples.class.getName()).log(Level.WARNING,String.format("%s had an error: %s",id,e.getLocalizedMessage()));
            return null;
        }
    }

    @Override
    public AudioSamples prototype(Properties props){
        String id;
        if(props.quickCheck("ID",String.class)){
            id = (String)props.quickGet("ID");
        }else{
            id = Double.toString(Math.random());
        }

        double[][] channels;
        if(props.quickCheck("Channels",double[][].class)){
            channels = (double[][])props.quickGet("Channels");
        }else{
            Logger.getLogger(AudioSamples.class.getName()).log(Level.SEVERE,"No data was provided");
            return null;
        }

        float samplingRate;
        if(props.quickCheck("SamplingRate",float.class)){
            samplingRate = (float)props.quickGet("SamplingRate");
        }else{
            samplingRate = 44100.0f;
        }
        return prototype(id,channels,samplingRate,props);
    }
